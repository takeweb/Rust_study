fn get_n_diverged(x0: f64, y0: f64, max_iter: usize) -> u8 {
    ...
    for i in 1..max_iter {
        ...
        yn = y_next;
        if yn * yn + xn * xn > 4.0 {
            return i as u8; // 複素数の絶対値が２を超えると発散と判断する
        }
    }
    max_iter as u8
}
...
pub fn generate_mandelbrot_set(
    ...
    // JavaScriptの８ビット符号なし整数の配列であるUint8ClampedArray型を作りたい為、Vec<u8>で色情報を作る
    let mut data = vec![];
    for i in 0..canvas_h {
        let i_f64 = i as f64;
