...
macro_rules! mesaure_elapsed_time {
    ($t:tt, $s:block) => {{
        let window = web_sys::window().expect("shoud have a window in the context");
        let performance = window
            .performance()
            .expect("performance shoud be available");
...
pub fn generate_mandelbrot_set(
    ...
    mesaure_elapsed_time!("genarate:wasm\telapsed:", {
...
pub fn draw_mandelbrot_set() {
    ...
    let mut result = mesaure_elapsed_time!("generate:::wasm\telapsed:", {
        logic::generate_mandelbrot_set(canvas_w, canvas_h,
            X_MIN, X_MAX, Y_MIN, Y_MAX, MAX_ITER)
    });

    mesaure_elapsed_time!("draw:wasm\telapsed:", {
